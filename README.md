...
Compilation of personal Ultimaker Cura postprocessing scripts


Installation
--

The files must be stored in the user script directory of the respective Cura version: **\AppData\Roaming\cura\ (current version of cura)\scripts**
...
![SpeedTower.py](./images/speedtower.jpg)
 ...
 This script offers the possibilities to modify :
...
- Possibility to define also a Fan Tower, Fan percentage speed indicate with a semi-colon as separator
...
Note: Some ironing areas are not detected by the script. It is mainly the small areas located in contour areas.
...
Description: add header info and part thumbnail for RepRap machine 3DWOX  
...
GradientInfill.py Postprocessing Script for Cura Plugin. Save the file in the _C:\Program Files\Ultimaker Cura **X.X**\plugins\PostProcessingPlugin\scripts_ directory.
...
No Gcode will be generated by Cura in this case. Same behavior if Cura settings are not suitable for Gradient Infill modification :

- Infill pattern types ZigZag, Concentric, Cross, and Cross3D are not allowed  
- In Cura, the option "Connect Infill Lines" for the other patterns mustn't be used.
...
- Test with outer wall : "Test the gradient with the outer wall segments


A new Flow Value for a short distance (Linear move < 2 x Gradient distance) was added to the standard GradientInfill script.

Add a gradual speed variation for a machine without a direct drive extruder.
...
Insert a Gcode at Layer Number.  Can be inserted before or after the Layer Number.
...
Script to modify the first layer infill and Check the first Wall Speed Bug Cura 5.6 
...
![SlowZ.py](./images/slowz.jpg)
	...
	- Display details on LCD : This setting will insert M117 gcode instructions, to display the current modification in the G-Code being used.
...
![MultiBrim.py](./images/multibrim.jpg)
	...
	- Brim addition : Number of brims to add to the existing one.
...
Script Cura to change the FAN value when we are in the case of Minimum Layer Time. When we are in a minimum Layer Time situation the Script fixes the FAN values to the **Fan value in %** 
...
Description: Z hop for every G0 displacement even if Retraction is not defined ( Retraction must be deactivated). Use the speed_z_hop and the retraction_hop as retraction parameters.

![ZMoveG0.py](./images/ZmoveG0.jpg)

Errors messages if Retraction is activated.

![Error ZMoveG0](./images/ErrorZmoveG0.jpg)
